  ...
  fetch("https://api.le-systeme-solaire.net/rest/bodies?filter[]=isPlanet,eq,true&data=name,moons")
        ...
        .then(data => {
          ...
          for (let i = 0; i < 8; i++) {
            ...
            newPlanet = []
            newPlanet.push(data.bodies[i].name + " : ")
            newPlanet.push(moonsCount)
            order.push(newPlanet)
            //console.log(order)
            //document.getElementById("filter").innerHTML += order
            //document.getElementById("filter").innerHTML += data.bodies[i].name + " : " + moonsCount + "<br>";
          }
        ...
        */
        //order=moons.moon,asc
        fetch("https://api.le-systeme-solaire.net/rest/bodies?filter[]=isPlanet,eq,true&data=name&order=moons,asc")
        .then(function (response) {
          if (response.ok) {
            return response.json();
          }
